...
export default function RewardsPage() {
    ...
    const fetchUserData = async (userId: number) => {
        try {
            const [pointsRes, userRewardsRes, transactionsRes] = await Promise.all([
                api.get(`/api/v1/rewards/points/user/${userId}`),
                api.get(`/api/v1/rewards/user-rewards/${userId}`),
                api.get(`/api/v1/rewards/transactions/user/${userId}`)
            ]);
            // O api.get já retorna response.data
            setUserPoints(pointsRes.data || null);
            setUserRewards(Array.isArray(userRewardsRes.data) ? userRewardsRes.data : []);
    ...
    const handleAddReward = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await api.post('/api/v1/rewards', selectedReward);
    ...
    const handleDeactivateReward = async (rewardId: number) => {
        try {
            await api.delete(`/api/v1/rewards/${rewardId}`);
            fetchData();
        } catch (err) {
    ...
    const handleEarnPoints = async (userId: number, points: number) => {
        try {
            await api.post('/api/v1/rewards/points/earn', {
                user_id: userId,
                points: points,
                            ...
                            <div key={reward.id} className="border rounded-lg p-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span>{reward.current_redemptions}/{reward.max_redemptions || '∞'}</span>
                                    </div>
