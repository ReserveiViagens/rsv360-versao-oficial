...
export default function Sidebar({ isOpen, onToggle, isCollapsed, onToggleCollapse }: SidebarProps) {
  ...
  const menuItems: MenuItem[] = [
    ...
    },
    {
      ...
      submenu: [
        { id: 'settings-general', name: 'Configurações Gerais', icon: <Settings className="h-4 w-4" />, href: '/settings' },
        { id: 'settings-system', name: 'Sistema', icon: <Server className="h-4 w-4" />, href: '/configuracoes-sistema' },
        { id: 'settings-apis', name: 'APIs e Mapas', icon: <Globe className="h-4 w-4" />, href: '/integracoes-apis' },
        { id: 'settings-users', name: 'Usuários', icon: <Users className="h-4 w-4" />, href: '/configuracoes-usuarios' }
      ]
    }
  ];
