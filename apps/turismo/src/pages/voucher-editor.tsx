...
export default function VoucherEditor() {
  ...
  const handleFocusCapture = useCallback((event: React.FocusEvent<HTMLElement>) => {
    ...
    if (target && typeof target.name === 'string' && target.name) {
      activeFieldNameRef.current = target.name;
    }
  }, []);

  useEffect(() => {
    if (showHeaderEditor || showBodyEditor || showFooterEditor) {
      restoreFocus();
    }
  }, [headerData, bodyData, footerData, showHeaderEditor, showBodyEditor, showFooterEditor, restoreFocus]);

  useEffect(() => {
    if (!showHeaderEditor && !showBodyEditor && !showFooterEditor) {
      activeFieldNameRef.current = null;
    }
  }, [showHeaderEditor, showBodyEditor, showFooterEditor]);

  const restoreFocus = useCallback(() => {
    const fieldName = activeFieldNameRef.current;
    if (!fieldName) return;
    const element = document.querySelector<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>(`[name="${fieldName}"]`);
  ...
  const HeaderEditor = () => (
    <div className="bg-white rounded-lg shadow p-6" onFocusCapture={handleFocusCapture}>
