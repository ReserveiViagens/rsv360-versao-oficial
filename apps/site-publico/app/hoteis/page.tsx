...
import { HotelComparison } from "@/components/hotel-comparison"
import { RemoteState } from "@/components/ui/remote-state"
...
const convertApiHotelToHotel = (apiHotel: ApiHotel): any => {
  ...
  // Determinar categoria baseado em estrelas e preço
  let category: 'luxury' | 'premium' | 'standard' | 'budget' = 'standard';
  if (stars >= 5 || price >= 500) category = 'luxury';
  else if (stars >= 4 || price >= 300) category = 'premium';
  else if (price < 150) category = 'budget';

  // Coordenadas padrão para Caldas Novas
  const coordinates = {
    lat: -17.7444 + (Math.random() * 0.1 - 0.05), // Variação pequena
    lng: -48.6278 + (Math.random() * 0.1 - 0.05)
  };
  ...
  return {
    ...
    distanceFromCenter,
    features,
    amenities: features, // Adicionar amenities também
    maxGuests,
    discount: discountPercent,
    category,
    coordinates,
    availability: {
      status: apiHotel.status === 'active' ? 'available' : 'sold_out' as 'available' | 'limited' | 'sold_out' | 'coming_soon',
      lastUpdated: new Date().toISOString(),
      roomsAvailable: Math.floor(Math.random() * 20) + 5 // Simular disponibilidade
    },
  };
};
...
export default function HoteisPage() {
  ...
  const { data: apiHotels, isLoading: isLoadingData, isRefreshing, error: dataError, retry } = useWebsiteContent('hotel');
  ...
  const [selectedHotelForDetails, setSelectedHotelForDetails] = useState<any | null>(null)
  const [comparisonHotels, setComparisonHotels] = useState<any[]>([])
    ...
    return (
      <div className="max-w-md mx-auto bg-gray-50 min-h-screen relative">
        <div className="flex flex-col items-center justify-center min-h-screen p-6">
          <RemoteState
            status="error"
            error="Nao foi possivel carregar os hoteis no momento."
            onRetry={retry}
          >
            <div />
          </RemoteState>
      ...
      <div className="max-w-md mx-auto bg-gray-50 min-h-screen relative">
        <div className="flex flex-col items-center justify-center min-h-screen p-6">
          <RemoteState
            status="success"
            empty
            emptyTitle="Nenhum hotel configurado ainda"
            emptyDescription="No momento nao temos hoteis disponiveis. Tente novamente mais tarde."
            onRetry={retry}
          >
            <div />
          </RemoteState>
    ...
    <div className="max-w-md mx-auto bg-gray-50 min-h-screen relative">
      <div className="animate-in fade-in duration-500">
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Cross-sell: após lista — 1 card Viagens em grupo */}
          <div>
      ...
      {selectedHotelForDetails && (
        <HotelDetailsModal
          ...
          allHotels={hotels}
          onCompare={(hotelsToCompare) => {
            setComparisonHotels(hotelsToCompare)
      ...
      {/* Modal de Comparação */}
      {comparisonOpen && comparisonHotels.length > 0 && (
        <HotelComparison
