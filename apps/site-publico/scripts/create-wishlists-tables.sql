-- ✅ ITEM 12: TABELAS DE WISHLISTS COMPARTILHADAS
...
CREATE TABLE IF NOT EXISTS shared_wishlists (
  ...
  -- Referências (flexível - pode ser users ou customers)
...
CREATE TABLE IF NOT EXISTS wishlist_members (
  ...
  -- Permite membro por user_id OU email (para convites pendentes)
  -- Constraint única será aplicada via trigger ou lógica de aplicação
);
...
CREATE TABLE IF NOT EXISTS wishlist_items (
  id SERIAL PRIMARY KEY,
  wishlist_id INTEGER NOT NULL REFERENCES shared_wishlists(id) ON DELETE CASCADE,
  item_id INTEGER NOT NULL, -- Referência a content(id) ou items(id)
  item_type VARCHAR(50) DEFAULT 'property', -- 'property', 'hotel', 'attraction', etc.
  added_by INTEGER,
  notes TEXT,
  check_in DATE,
  check_out DATE,
...
-- 4. ✅ ITEM 14: Votos (Sistema de Votação)
CREATE TABLE IF NOT EXISTS wishlist_votes (
  ...
  -- Permite voto por user_id OU email
  -- Constraint única será aplicada via trigger ou lógica de aplicação
);
...
-- Comentários
COMMENT ON TABLE shared_wishlists IS 'Wishlists compartilhadas para viagens em grupo';
COMMENT ON TABLE wishlist_members IS 'Membros e permissões das wishlists';
COMMENT ON TABLE wishlist_items IS 'Items (propriedades/hotéis) adicionados às wishlists';
COMMENT ON TABLE wishlist_votes IS 'Sistema de votação para items da wishlist';
