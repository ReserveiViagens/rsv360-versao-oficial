...
-- 1. Tabela principal de divisão de pagamentos
...
-- 2. Participantes do split (quem deve pagar)
CREATE TABLE IF NOT EXISTS split_payment_participants (
  ...
  -- Permite participante por user_id OU email
  CONSTRAINT unique_participant UNIQUE(split_payment_id, COALESCE(user_id::text, email))
);
...
-- 3. Histórico de pagamentos do split
...
-- Trigger para atualizar status do split automaticamente
...
BEGIN
  -- Buscar valores do split
  SELECT 
    sp.total_amount,
    COUNT(DISTINCT spp.id) as total,
    COUNT(DISTINCT CASE WHEN spp.status = 'paid' THEN spp.id END) as paid
  INTO total_amount, total_participants, paid_participants
  FROM split_payments sp
  LEFT JOIN split_payment_participants spp ON sp.id = spp.split_payment_id
  WHERE sp.id = NEW.split_payment_id
  GROUP BY sp.id, sp.total_amount;

  -- Calcular total pago
  SELECT COALESCE(SUM(amount), 0)
  INTO total_paid
  FROM split_payment_participants
  WHERE split_payment_id = NEW.split_payment_id AND status = 'paid';
...
COMMENT ON TABLE split_payment_participants IS 'Participantes que devem pagar parte do valor';
COMMENT ON TABLE split_payment_history IS 'Histórico de pagamentos individuais';
