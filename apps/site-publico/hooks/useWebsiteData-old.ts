...
export function useWebsiteData() {
  ...
  const loadSettings = async () => {
    try {
      const data = await fetchData(`${API_BASE_URL}/api/admin/website/settings`);
  ...
  const createHotel = async (hotelData: Omit<Hotel, 'id'>) => {
    try {
      const newHotel = await fetchData(`${API_BASE_URL}/api/admin/website/content`, {
        ...
        body: JSON.stringify({
          page_type: 'hotels',
          content_id: hotelData.title.toLowerCase().replace(/\s+/g, '-'),
          ...hotelData
        }),
      });
  ...
  const updateHotel = async (id: number, hotelData: Partial<Hotel>) => {
    try {
      ...
      const contentId = hotel.title.toLowerCase().replace(/\s+/g, '-');
      const updatedHotel = await fetchData(`${API_BASE_URL}/api/admin/website/content/hotels/${contentId}`, {
        method: 'PUT',
  ...
  const deleteHotel = async (id: number) => {
    try {
      ...
      const contentId = hotel.title.toLowerCase().replace(/\s+/g, '-');
      await fetchData(`${API_BASE_URL}/api/admin/website/content/hotels/${contentId}`, {
        method: 'DELETE',
      });
    ...
    try {
      const newPromotion = await fetchData(`${API_BASE_URL}/api/admin/website/content`, {
        ...
        body: JSON.stringify({
          ...
          content_id: promotionData.title.toLowerCase().replace(/\s+/g, '-'),
          ...promotionData
        }),
      });
  ...
  const updatePromotion = async (id: number, promotionData: Partial<Promotion>) => {
    try {
      ...
      const contentId = promotion.title.toLowerCase().replace(/\s+/g, '-');
      const updatedPromotion = await fetchData(`${API_BASE_URL}/api/admin/website/content/promotions/${contentId}`, {
        method: 'PUT',
  ...
  const deletePromotion = async (id: number) => {
    try {
      await fetchData(`${API_BASE_URL}/api/admin/website/content/${id}`, {
        method: 'DELETE',
      });
  ...
  const createAttraction = async (attractionData: Omit<Attraction, 'id'>) => {
    try {
      ...
      setAttractions(prev => [...prev, newAttraction.content]);
      return newAttraction.content;
    } catch (err) {
  ...
  const updateAttraction = async (id: number, attractionData: Partial<Attraction>) => {
    try {
      const updatedAttraction = await fetchData(`${API_BASE_URL}/api/admin/website/content/${id}`, {
        method: 'PUT',
        body: JSON.stringify(attractionData),
      });

      setAttractions(prev => prev.map(attraction =>
        attraction.id === id ? updatedAttraction.content : attraction
      ));
      return updatedAttraction.content;
    } catch (err) {
