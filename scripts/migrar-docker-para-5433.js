...
async function obterEstruturaCompletaTabela(pool, tableName) {
  try {
    ...
    return {
      ...
      comment: comments.rows[0]?.table_comment || null,
    };
  } catch (error) {
...
async function gerarSQLCriacaoTabela(pool, tableName) {
  ...
  for (const uq of uniqueConstraints) {
    sql += `ALTER TABLE "${tableName}" ADD CONSTRAINT "${uq.constraint_name}" UNIQUE ("${uq.column_name}");\n`;
  }
  ...
  for (const idx of indexConstraints) {
    ...
    indexDef = indexDef.replace(/CREATE\s+UNIQUE\s+INDEX\s+\w+/, `CREATE INDEX ${newIndexName}`);
...
async function migrarDados(sourcePool, targetPool, tableName) {
  try {
    ...
    if (!targetExists.rows[0].exists) {
      console.log(`  ⚠️  Tabela ${tableName} não existe no destino. Criando estrutura primeiro...`);
    ...
    if (existingCount > 0) {
      console.log(`  ⚠️  Tabela ${tableName} já possui ${existingCount} registros. Pulando dados.`);
      ...
      for (const row of batch) {
        ...
        try {
          ...
          if (errors <= 5) { // Mostrar apenas os primeiros 5 erros
            console.error(`    ⚠️  Erro ao inserir registro ${inserted + errors} em ${tableName}:`, error.message);
          }
        }
      }
    }
    
    return { inserted, skipped: count - inserted - errors, errors };
  } catch (error) {
...
async function executarMigracao() {
  ...
  if (!dockerAcessivel) {
    ...
    console.log('  ⚠️  Verifique se o container está rodando:');
    console.log('     docker ps | findstr postgres-rsv360');
    console.log('');
    return;
  }
  ...
  try {
    const tabelasParaMigrar = comparacao.faltam.length > 0 
      ? comparacao.faltam 
      : TABELAS_DOCKER; // Se todas existem, migrar dados de todas
    ...
    for (const tableName of TABELAS_DOCKER) {
    ...
    relatorio += `| Tabela | Criada | Registros | Erros |\n`;
    relatorio += `|--------|--------|-----------|-------|\n`;
    resultados.detalhes.forEach(d => {
      relatorio += `| ${d.tabela} | ${d.criada ? '✅' : '❌'} | ${d.registros} | ${d.erros} |\n`;
    });
